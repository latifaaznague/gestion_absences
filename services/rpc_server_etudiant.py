...
def get_db_connection():
    ...
    try:
        conn = psycopg2.connect(
            ...
            user="postgres",
            password="emabk",  # Votre mot de passe actuel
            port=5432,
    ...
    try:
        ...
        return Success({
            ...
            "etudiant": {
                ...
                "dernier_seuil_alerte": etudiant["dernier_seuil_alerte"],
                "promotion": {
...
@method
def verifier_et_creer_notifications(etudiant_id: int):
    """Vérifie et crée des notifications pour l'étudiant"""
    logger.info(f"Vérification des notifications pour l'étudiant ID: {etudiant_id}")
    
    try:
        # 1. Vérifier les justifications en attente
        query_justifications = """
            SELECT 
                p.id as presence_id,
            ...
            JOIN cours c ON s.cours_id = c.id
            WHERE p.etudiant_id = %s
            AND p.statut_justification IN ('ACCEPTEE', 'REFUSEE')
            AND NOT EXISTS (
                SELECT 1 FROM notification n 
                WHERE n.presence_id = p.id 
                AND n.etudiant_id = p.etudiant_id
            )
        """
        
        justifications = execute_query_all(query_justifications, (etudiant_id,))
        
        notifications_crees = 0
        
        # Créer des notifications pour chaque justification traitée
        for j in justifications:
            if j["statut_justification"] == "ACCEPTEE":
                message = f"✅ Votre justification pour le cours '{j['cours_libelle']}' du {j['date'].strftime('%d/%m/%Y')} a été acceptée."
            else:  # REFUSEE
                message = f"❌ Votre justification pour le cours '{j['cours_libelle']}' du {j['date'].strftime('%d/%m/%Y')} a été refusée."
            
            # Créer la notification
            insert_query = """
                INSERT INTO notification (etudiant_id, presence_id, message, date_envoi, lu, type_notification)
                VALUES (%s, %s, %s, NOW(), FALSE, %s)
            """
            
            notification_type = 'JUSTIFICATION_ACCEPTEE' if j["statut_justification"] == 'ACCEPTEE' else 'JUSTIFICATION_REFUSEE'
            execute_update(insert_query, (etudiant_id, j["presence_id"], message, notification_type))
            notifications_crees += 1
        
        # 2. Vérifier si l'étudiant dépasse 3 absences
        query_absences = """
        ...
        absences = execute_query(query_absences, (etudiant_id,))
        total_absences = absences["total_absences"] if absences else 0
        
        # Récupérer le dernier seuil d'alerte
        seuil_query = "SELECT dernier_seuil_alerte FROM etudiant WHERE id = %s"
        seuil_result = execute_query(seuil_query, (etudiant_id,))
        dernier_seuil = seuil_result["dernier_seuil_alerte"] if seuil_result else 0
        
        # Si plus de 3 absences ET dernier seuil inférieur aux absences actuelles
        if total_absences > 3 and dernier_seuil < total_absences:
            # Vérifier si une notification d'alerte existe déjà pour ce seuil
            check_alerte_query = """
                SELECT COUNT(*) as count
                FROM notification n
                WHERE n.etudiant_id = %s
                AND n.type_notification = 'ALERTE_ABSENCES'
                AND n.message LIKE '%Vous avez dépassé%'
                AND n.date_envoi >= NOW() - INTERVAL '1 day'
            """
            
            exists = execute_query(check_alerte_query, (etudiant_id,))
            existing_count = exists["count"] if exists else 0
            
            if existing_count == 0:
                message = f"⚠️ ALERTE ABSENCES: Vous avez dépassé {total_absences} absences.\n"
                message += f"Vous devez contacter rapidement l'administration."
                
                insert_query = """
                    INSERT INTO notification (etudiant_id, message, date_envoi, lu, type_notification)
                    VALUES (%s, %s, NOW(), FALSE, 'ALERTE_ABSENCES')
                """
                
                execute_update(insert_query, (etudiant_id, message))
                notifications_crees += 1
                
                # Mettre à jour le dernier seuil d'alerte
                update_seuil_query = """
                    UPDATE etudiant 
                    SET dernier_seuil_alerte = %s 
                    WHERE id = %s
                """
                execute_update(update_seuil_query, (total_absences, etudiant_id))
        
        logger.info(f"Notifications créées : {notifications_crees}")
        
        return Success({
            "status": "success",
            "notifications_crees": notifications_crees,
...
@method
def get_cours_etudiant(etudiant_id: int):
    """Récupère tous les cours d'un étudiant"""
    logger.info(f"Récupération des cours pour l'étudiant ID: {etudiant_id}")
    ...
    try:
        query = """
            SELECT DISTINCT
                ...
                p.specialite as professeur_specialite
            FROM cours c
            JOIN seance s ON c.id = s.cours_id
            JOIN groupe g ON s.groupe_id = g.id
            JOIN etudiant_groupe eg ON g.id = eg.groupe_id
            LEFT JOIN professeur p ON c.professeur_id = p.utilisateur_id
            LEFT JOIN utilisateur u ON p.utilisateur_id = u.id
            WHERE eg.etudiant_id = %s
            ORDER BY c.libelle
        """
        
        cours = execute_query_all(query, (etudiant_id,))
        ...
        for c in cours:
            result.append({
                ...
                "professeur": {
                    "nom": c["professeur_nom"],
                    "specialite": c["professeur_specialite"]
                } if c["professeur_nom"] else None
            })
...
@method
def get_planning_mensuel(etudiant_id: int, annee: int, mois: int):
    """Récupère le planning mensuel d'un étudiant"""
    logger.info(f"Récupération du planning mensuel pour l'étudiant ID: {etudiant_id}, {mois}/{annee}")
    ...
    try:
        ...
        if mois == 12:
            end_date = datetime(annee + 1, 1, 1).date() - timedelta(days=1)
        else:
            end_date = datetime(annee, mois + 1, 1).date() - timedelta(days=1)
        ...
        query = """
            ...
            JOIN groupe g ON s.groupe_id = g.id
            JOIN etudiant_groupe eg ON g.id = eg.groupe_id
            LEFT JOIN presence p ON s.id = p.seance_id AND p.etudiant_id = %s
            WHERE eg.etudiant_id = %s 
            AND s.date BETWEEN %s AND %s
            ORDER BY s.date, s.heure_debut
        """
        
        seances = execute_query_all(query, (etudiant_id, etudiant_id, start_date, end_date))
        ...
        for s in seances:
            result.append({
                ...
                "presence": {
                    "statut": s["statut"],
                    "statut_justification": s["statut_justification"]
                } if s["statut"] else None
            })
        
        logger.info(f"Nombre de séances ce mois-ci: {len(result)}")
        ...
        logger.error(f"Erreur lors de la récupération du planning mensuel: {str(e)}")
        return Error(code=500, message=f"Erreur serveur: {str(e)}")
...
def test_endpoint():
    ...
    return json.dumps({
        "message": "Serveur RPC Flask fonctionnel",
        "timestamp": datetime.now().isoformat(),
        "endpoints": [
            {"method": "POST", "path": "/", "description": "Endpoint RPC principal"},
            {"method": "GET", "path": "/health", "description": "Vérification santé"},
            {"method": "GET", "path": "/test", "description": "Test simple"}
        ]
    }), 200, {"Content-Type": "application/json"}

if __name__ == "__main__":
    logger.info("=" * 50)
    logger.info("Démarrage du serveur RPC Flask pour Gestion Absences")
    logger.info("=" * 50)
    ...
    try:
        ...
        logger.info("🔧 Points d'entrée disponibles:")
        logger.info("   - POST / : Endpoint RPC principal")
        logger.info("   - GET /health : Vérification santé")
        logger.info("   - GET /test : Test simple")
        logger.info("=" * 50)
