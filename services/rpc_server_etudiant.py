    ...
    try:
        ...
        for j in justifications:
            if j["statut_justification"] == "ACCEPTEE":
                ...
                if j["feedback_professeur"]:
                    message += f" Feedback : {j['feedback_professeur']}"
            else:  # REFUSEE
            ...
            insert_query = """
                INSERT INTO notification (etudiant_id, presence_id, message, date_envoi, lu)
                VALUES (%s, %s, %s, NOW(), FALSE)
            """
            
            execute_update(insert_query, (etudiant_id, j["presence_id"], message))
        ...
        query_absences = """
            ...
            AND p.seance_id IN (
                ...
                WHERE s.date >= DATE_TRUNC('month', NOW() - INTERVAL '1 month')
            )
        """
        ...
        # Si plus de 3 absences, créer une notification
        if total_absences >= 3:
            ...
            check_query = """
                ...
                FROM notification n
                WHERE n.etudiant_id = %s
                AND n.message LIKE 'Alerte : Vous avez dépassé%'
                AND n.date_envoi >= NOW() - INTERVAL '7 days'
            """
            
            exists = execute_query(check_query, (etudiant_id,))
            existing_count = exists["count"] if exists else 0
            
            if existing_count == 0:
                message = f"Alerte : Vous avez dépassé {total_absences} absences ce mois-ci. Consultez vos absences."
                
                insert_query = """
                    INSERT INTO notification (etudiant_id, message, date_envoi, lu)
